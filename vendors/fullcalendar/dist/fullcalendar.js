...
function computeIsLeftRtlScrollbars() { // creates an offscreen admin element, then removes it
	var el = $('<div><div/></div>')
		.css({
			position: 'absolute',
			top: -1000,
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	bindEvents: function() {
		...
		if (!this.isEventsBound) {
			...
			this.rejectOn('eventsUnbind', this.requestEvents()).then(function(events) { // TODO: admin rejection
...
Calendar.prototype.isSpanAllowed = function(span, constraint, overlap, event) {
	...
	for (i = 0; i < peerEvents.length; i++)  {
		...
		if (this.eventIntersectsRange(peerEvent, span)) {
			...
			// if the event's overlap is a admin function, pass the peer event in question as the first param
			...
			if (event) {
				...
				// if the peer event's overlap is a admin function, pass the subject event as the first param
