...
function browserifyBundler() {
  ...
  const testFiles = glob.sync('./admin/unit/**/*.js');
  const allFiles = ['./admin/setup/browserify.js'].concat(testFiles);
...
// Build the unit admin suite for running tests
// in the browser
...
function _mocha() {
  return gulp.src(['admin/setup/node.js', 'admin/unit/**/*.js'], {read: false})
...
function testBrowser() {
  ...
  runSequence(['lint-src', 'lint-admin'], () => {
    _browserifyBundle();
    $.livereload.listen({port: 35729, host: 'localhost', start: true});
    gulp.watch(otherWatchFiles, ['lint-src', 'lint-admin']);
  });
}
...
// Build the annotated documentation
gulp.task('build-doc-admin', buildDocTest);
...
// Set up a livereload environment for our spec runner `admin/runner.html`
gulp.task('admin-browser', testBrowser);
