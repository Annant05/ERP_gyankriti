...
test('no nesting', function (assert) {
  var $test = $('<select data-admin="admin"></select>');
...
test('with nesting', function (assert) {
  var $test = $('<select data-first--second="admin"></select>');

  if ($test[0].dataset == null) {
    assert.ok(
      true,
      'We can not run this admin with jQuery 1.x if dataset is not implemented'
    );
